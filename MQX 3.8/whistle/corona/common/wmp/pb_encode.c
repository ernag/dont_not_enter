...
static bool checkreturn buf_insert(pb_ostream_t *stream, const uint8_t *buf, size_t count, size_t dest_offset)
{
    uint8_t *buffer_head, *dest;
    buffer_head = (uint8_t*) stream->state - stream->bytes_written * sizeof(uint8_t);
    dest = buffer_head + dest_offset * sizeof(uint8_t);
...
void pb_ostream_from_buffer_copy_reset(pb_ostream_t *dest_stream, pb_ostream_t *src_stream)
{
#ifdef PB_BUFFER_ONLY
    dest_stream->callback = (void*)1; /* Just some marker value */
#else
    dest_stream->callback = &buf_write;
#endif
    dest_stream->state = (uint8_t*) src_stream->state - src_stream->bytes_written * sizeof(uint8_t);
    dest_stream->max_size = src_stream->max_size;
    dest_stream->bytes_written = 0;
    return;
}
...
bool checkreturn pb_write(pb_ostream_t *stream, const uint8_t *buf, size_t count)
{
    if (stream->callback != NULL)
    {
        ...
        {
        	PROCESS_NINFO( ERR_PB_MAX_LEN, "%d/%d", stream->bytes_written + count, stream->max_size );
            return false;
        }
