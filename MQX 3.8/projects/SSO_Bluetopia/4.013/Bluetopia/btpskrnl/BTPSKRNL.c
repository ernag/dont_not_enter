...
#define MAX_NUMBER_SUPPORTED_TASKS                          5
   ...
   /* The following values are the values that are used when creating a */
   /* Thread using the BTPS_CreateThread() function.  Threads are       */
   /* created in MQX as Time Sliced so that they multi-task.            */
#define DEFAULT_BTPS_TASK_PRIORITY                          5
#define DEFAULT_BTPS_TIME_SLICE_VALUE                       5
...
static TASK_TEMPLATE_STRUCT          MQX_Task_List[MAX_NUMBER_SUPPORTED_TASKS + 1];
static unsigned int                  NumberActiveTasks;
...
static void ThreadWrapper(uint_32 UserData)
{
   if(UserData)
   {
      ...
      /* Free the Thread Wrapper memory.                                */
      BTPS_FreeMemory((void *)UserData);

      /* Delete this thread.                                            */
      _task_destroy(MQX_NULL_TASK_ID);

      /* Delay forever if _task_destroy fails.                          */
   ...
   if(Mutex)
   {
      ...
      else
      {
         if(Timeout == BTPS_INFINITE_WAIT)
         {
            ...
            /* for this.                                                */
            if(_mutex_lock(&(((MutexHeader_t *)Mutex)->Mutex)) == MQX_EOK)
               ret_val = TRUE;
            else
               ret_val = FALSE;
         }
...
Boolean_t BTPSAPI BTPS_WaitEvent(Event_t Event, unsigned long Timeout)
{
   ...
   if(Event)
   {
      if(Timeout == BTPS_INFINITE_WAIT)
      {
         ...
         if(_lwevent_wait_for((LWEVENT_STRUCT_PTR)Event, 1, (boolean)FALSE, NULL) == MQX_EOK)
            ret_val = TRUE;
         else
            ret_val = FALSE;
      }
      else
      {
         ...
         if(_lwevent_wait_for((LWEVENT_STRUCT_PTR)Event, 1, (boolean)FALSE, &TickTimeout) == MQX_EOK)
            ret_val = TRUE;
         else
            ret_val = FALSE;
      }
...
void *BTPSAPI BTPS_AllocateMemory(unsigned long MemorySize)
{
   ...
   if(MemorySize)
   {
      ...
      ret_val = (void *)_mem_alloc_system(MemorySize);

      if(((DWord_t)ret_val) & 0x00000003)
   ...
   if(!ret_val)
      DBG_MSG(DBG_ZONE_CRITICAL_ERROR, ("Malloc %d bytes failed\r\n", MemorySize));
...
void BTPSAPI BTPS_FreeMemory(void *MemoryPointer)
{
   ...
   if(MemoryPointer)
      _mem_free((pointer)MemoryPointer);
   else
      DBG_MSG(DBG_ZONE_CRITICAL_ERROR,("Invalid Pointer\r\n"));
}
...
ThreadHandle_t BTPSAPI BTPS_CreateThread(Thread_t ThreadFunction, unsigned int StackSize, void *ThreadParameter)
{
   ...
   /* Make sure that the input parameters are valid AND we can create   */
   /* another Task.                                                     */
   if((ThreadFunction) && (StackSize) && (NumberActiveTasks < MAX_NUMBER_SUPPORTED_TASKS))
   {
      ...
      if((ThreadWrapperInfo = (ThreadWrapperInfo_t *)BTPS_AllocateMemory(sizeof(ThreadWrapperInfo_t))) != NULL)
      {
         ...
         BTPS_SprintF(ThreadWrapperInfo->ThreadName, "BT%d", NumberActiveTasks);

         /* Fill in the Task Entry.                                     */
         MQX_Task_List[NumberActiveTasks].TASK_TEMPLATE_INDEX = (NumberActiveTasks + 1)*2;
         MQX_Task_List[NumberActiveTasks].TASK_ADDRESS        = ThreadWrapper;
         MQX_Task_List[NumberActiveTasks].TASK_STACKSIZE      = StackSize;
         MQX_Task_List[NumberActiveTasks].TASK_PRIORITY       = DEFAULT_BTPS_TASK_PRIORITY;
         MQX_Task_List[NumberActiveTasks].TASK_NAME           = ThreadWrapperInfo->ThreadName;
         MQX_Task_List[NumberActiveTasks].CREATION_PARAMETER  = (uint_32)ThreadWrapperInfo;
         MQX_Task_List[NumberActiveTasks].DEFAULT_TIME_SLICE  = DEFAULT_BTPS_TIME_SLICE_VALUE;

#if MQX_HAS_TIME_SLICE

         MQX_Task_List[NumberActiveTasks].TASK_ATTRIBUTES     = MQX_TIME_SLICE_TASK;

#else

         MQX_Task_List[NumberActiveTasks].TASK_ATTRIBUTES     = 0;

#endif

         TaskID = _task_create(0, 0, (uint_32)&(MQX_Task_List[NumberActiveTasks]));
         if(TaskID > 0)
         {
            NumberActiveTasks++;

            ret_val = (ThreadHandle_t)TaskID;
         }
         else
         {
            /* Print out a debug message telling use we failed to create*/
            /* the specified task.                                      */
            DBG_MSG(DBG_ZONE_DEVELOPMENT,("Failed to create thread (Function @ 0x%08X), return %d.\n", ThreadFunction, TaskID));

            /* Error, go ahead and free the memory that was allocated.  */
