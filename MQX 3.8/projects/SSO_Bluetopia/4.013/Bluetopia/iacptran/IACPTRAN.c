...
#define COPROCESSOR_WRITE_ADDRESS (0x20 >> 1)
...
static FILE *I2CDeviceFilePtr = NULL;
...
static Boolean_t CheckAckFromCoprocessor(void)
{
   ...
   if(ioctl(I2CDeviceFilePtr, IO_IOCTL_FLUSH_OUTPUT, &Acknowledge) == I2C_OK)
   {
...
{
   /* Open the I2C device.  The MQX Board Support Package handles       */
   /* installing the driver simply by enabling the driver in            */
   /* user_config.h MQX header file.                                    */
   I2CDeviceFilePtr = fopen("i2c0:", NULL);

   /* If the we were able to open the device return 0 (success), else   */
   /* return that an open failure occurred.                             */
   return(I2CDeviceFilePtr ? 0 : IACP_OPEN_FAILURE);
}
...
void BTPSAPI IACPTR_Cleanup(void)
{
   if(I2CDeviceFilePtr)
   {
      fclose(I2CDeviceFilePtr);
      I2CDeviceFilePtr = NULL;
   }
}
...
int BTPSAPI IACPTR_Read(unsigned char Register, unsigned char BytesToRead, unsigned char *ReadBuffer, unsigned char *Status)
{
   ...
   ioctl(I2CDeviceFilePtr, IO_IOCTL_I2C_SET_DESTINATION_ADDRESS, &Param);

   /* Verify that the parameters passed in appear valid.                */
   if((BytesToRead) && (ReadBuffer) && (Status) && (I2CDeviceFilePtr))
   {
      ...
      *Status = IACP_STATUS_READ_FAILURE;
      ...
      do
      {
         if(NumAttempts > 0)
         {
            ...
            ioctl(I2CDeviceFilePtr, IO_IOCTL_I2C_REPEATED_START, NULL);
         }

         /* Send the write address of the coprocessor by writing 0 bytes*/
         /* to the device driver.                                       */
         (void)fwrite(&Param, 1, 0, I2CDeviceFilePtr);
      ...
      if(Acknowledged)
      {
         ...
         (void)fwrite(&Register, 1, 1, I2CDeviceFilePtr);
         ...
         do
         {
            ...
            Param = BytesToRead;
            ioctl(I2CDeviceFilePtr, IO_IOCTL_I2C_SET_RX_REQUEST, &Param);
            ...
            NumRead = fread(ReadBuffer, 1, BytesToRead, I2CDeviceFilePtr);
         ...
         ioctl(I2CDeviceFilePtr, IO_IOCTL_I2C_STOP, NULL);
...
int BTPSAPI IACPTR_Write(unsigned char Register, unsigned char BytesToWrite, unsigned char *WriteBuffer, unsigned char *Status)
{
   ...
   if(TempBuffer)
   {
      ...
      ioctl(I2CDeviceFilePtr, IO_IOCTL_I2C_SET_DESTINATION_ADDRESS, &Address);
      ...
      do
      {
         ...
         /* return 0.                                                   */
         NumWritten = fwrite(TempBuffer, 1, BytesToWrite, I2CDeviceFilePtr);
