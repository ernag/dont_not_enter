...
#define ISPP_PACKET_TIMEOUT_DEFAULT                                800
...
#define ISPP_EVENT_DATA_SIZE                             (sizeof(ISPP_Event_Data_t))
   ...
   /*            can only be satisfied by Receiving iSPP Event Packets. */
   /*            A Deadlock WILL occur because NO iSPP Event Callbacks  */
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Close_Server_Port(unsigned int BluetoothStackID, unsigned int SerialPortID);
   ...
   /* will operating in WiAP mode.  FIDInfo is a pointer to the FID     */
   ...
   /* send in a single packet.  This function returns zero if           */
   /* successful, or a negative return value if there was an error.     */
BTPSAPI_DECLARATION int BTPSAPI ISPP_Open_Port_Request_Response(unsigned int BluetoothStackID, unsigned int SerialPortID, Boolean_t AcceptConnection, unsigned int FIDInfoLength, Byte_t *FIDInfo, unsigned int MaxRxPacketSize);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_ISPP_Open_Port_Request_Response_t)(unsigned int BluetoothStackID, unsigned int SerialPortID, Boolean_t AcceptConnection, unsigned int FIDInfoLength, Byte_t *FIDInfo, unsigned int MaxRxPacketSize);
#endif
...
#define ISPP_Un_Register_SDP_Record(__BluetoothStackID, __SerialPortID, __SDPRecordHandle) \
   ...
   /* function).                                                        */
BTPSAPI_DECLARATION int BTPSAPI ISPP_Open_Remote_Port(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, unsigned int ServerPort, unsigned int FIDInfoLength, Byte_t *FIDInfo, unsigned int MaxRxPacketSize, ISPP_Event_Callback_t ISPP_Event_Callback, unsigned long CallbackParameter);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_ISPP_Open_Remote_Port_t)(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, unsigned int ServerPort, unsigned int FIDInfoLength, Byte_t *FIDInfo, unsigned int MaxRxPacketSize, ISPP_Event_Callback_t ISPP_Event_Callback, unsigned long CallbackParameter);
#endif
...
BTPSAPI_DECLARATION int BTPSAPI ISPP_Open_Session_Request_Response(unsigned int BluetoothStackID, unsigned int SerialPortID, Word_t SessionID, Boolean_t Accept);
   ...
   /* of the transaction.                                               */
   /* * NOTE * The data is copied, queued and processed in FIFO order   */
   /*          and thus the data does not need to be maintained after   */
