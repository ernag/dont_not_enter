...
corona_err_t wson_comm_ok(void)
{
	...
	while(g_SPI2_Flash_Wait_for_Send)
	{
		if(msTimeout-- == 0)
		{
			printf("Timeout waiting for SPI FLASH send!!!\n");
	...
	while(g_SPI2_Flash_Wait_for_Receive)
	{
		if(msTimeout-- == 0)
		{
			printf("Timeout waiting for SPI FLASH receive!!!\n");
			return CC_ERROR_GENERIC;
		}
		sleep(1);   // \todo Fix sleeping here.
	}
	...
	if(rx[4] == 0x01)
	{
		printf("Manufacturer = 0x1 = SPANSION\n");
		if( (rx[5] != 0x17) && (rx[5] != 0x18) )
		{
			printf("ERROR: Did not recognized Spansion Device ID!\n");
			return CC_ERROR_GENERIC;
		}
		return SUCCESS;
	}
	...
	{
		printf("Manufacturer = 0x1 = WSON\n");
		if( (rx[5] != 0x17) && (rx[5] != 0x18) )
		{
			printf("ERROR: Did not recognized Winbond Device ID!\n");
			return CC_ERROR_GENERIC;
		}
		return SUCCESS;
	}
	...
	{
		printf("ERROR: Did not recognize Manufacturer ID\n");
		return CC_ERROR_GENERIC;
	}
...
corona_err_t wson_erase(uint32_t address)
{
	...
	sleep(1);  // \todo We probably need this sleep!
	...
	while(g_SPI2_Flash_Wait_for_Send){/* sleep */}
	g_SPI2_Flash_Wait_for_Send = TRUE;
	sleep(5); // \todo We probably need this sleep!
	
	err2 = wip_wait();
	if(err2 != ERR_OK)
	{
		printf("wip_wait() Error! ! 0x%x\n", err2);
		return err2;
	}
...
corona_err_t wson_flash_torture(void)
{
	...
	if( (!pData) || (!rx) )
	{
		printf("Memory allocation Error in Torture Test!!\n");
		while(1){}
	}
	
    for (address = EXT_FLASH_4KB_REGION; address < (EXT_FLASH_64KB_REGION+3*EXT_FLASH_64KB_SZ); address += EXT_FLASH_PAGE_SIZE)
    {
		...
		if(address >= EXT_FLASH_64KB_REGION)
        {
            ...
            {
                printf("ERASE-ing 64KB SECTOR: (0x%x), testing SECTOR's 256 PAGEs.\n", address);
                erase = 1;
            }
        }
        else if( (address % EXT_FLASH_4KB_SZ) == 0)
        {
            printf("ERASE-ing 4KB SECTOR: (0x%x), testing SECTOR's 16 PAGEs.\n", address);
            erase = 1;
        }
		...
		if(err != SUCCESS)
		{
			printf("Wson write() returned an error (0x%x)!\n", err);
			while(1){}
		}
		...
		if(err != SUCCESS)
		{
			printf("Wson read() returned an error (0x%x)!\n", err);
			while(1){}
		}
