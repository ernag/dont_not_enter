...
void TestApp_Init(void)
{
    ...
    if(error != USB_OK)
    {
        ...
        printf("Error initializing USB-CDC Class %d\n", error);
        return;
    }
    EnableInterrupts;
	#if (defined _MCF51MM256_H) || (defined _MCF51JE256_H)
     usb_int_en();
    #endif
    printf("Initialized USB-CDC Class, result = %d\n", error);
}
...
void TestApp_Task(void)
{
	...
	if(USB_PROCESS_PENDING())
	{
        ...
        USB_Class_CDC_Periodic_Task();  // nada
 ...
 *****************************************************************************/
// jim: add call to command parser here

uint_8 cmd_rcv_buf[128];
uint_8 cmd_rcv_size;
...
)
{
    ...
    {
        start_app=FALSE;
        // printf("USB_App_Callback: event_type == USB_APP_BUS_RESET\n");
    }
...
#endif
        start_app=TRUE;
        printf("USB_App_Callback: event_type == USB_APP_ENUM_COMPLETE\n");
    }
...
)
{
    ...
    if(start_app == TRUE)
    {
        ...
        {
            start_transactions = TRUE;
            printf("USB_Notify_Callback: event_type == USB_APP_CDC_CARRIER_ACTIVATED\n");
        }
        else if(event_type == USB_APP_CDC_CARRIER_DEACTIVATED)
        {
            //start_transactions = FALSE;
            printf("USB_Notify_Callback: event_type == USB_APP_CDC_CARRIER_DEACTIVATED\n");
        }
    }
    return;
}
