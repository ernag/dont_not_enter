/* MK60N512 eDMA- and DSPI-based driver */
...
/* Only writes the important DMA registers between transactions */
...
#define spi_dma_hijack OLD_ISR_DATA
...
#define SPI_DMA_MTX_CREATE(p) _lwsem_create(p, 1)
#define SPI_DMA_MTX_DESTROY   _lwsem_destroy
#define SPI_DMA_MTX_UNLOCK    _lwsem_post
#define SPI_DMA_MTX_LOCK      _lwsem_wait
#define SPI_DMA_MTX           LWSEM_STRUCT
  ...
  )
{
    ...
    DSPI_INFO_STRUCT_PTR            io_info_ptr = NULL;
    ...
    SPI_DMA_MTX_CREATE(&spidma->lock);
...
{
    return _io_spi_polled_install(
        identifier, _dspi_dma_init, _dspi_dma_deinit,
        _dspi_dma_rx, _dspi_dma_tx, _dspi_dma_ioctl, (pointer)init_data_ptr);
}
