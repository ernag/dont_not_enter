...
} I2C_Display_TDeviceData;

typedef I2C_Display_TDeviceData *I2C_Display_TDeviceDataPtr; /* Pointer to the device data structure. */

/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static I2C_Display_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
/* {Default RTOS Adapter} Global variable used for passing a parameter into ISR */
static I2C_Display_TDeviceDataPtr INT_I2C1__DEFAULT_RTOS_ISRPARAM;
...
PE_ISR(I2C_Display_Interrupt)
{
  ...
  I2C_Display_TDeviceDataPtr DeviceDataPrv = INT_I2C1__DEFAULT_RTOS_ISRPARAM;
  ...
  if (I2C_PDD_GetMasterMode(I2C1_BASE_PTR) == I2C_PDD_MASTER_MODE) { /* Is device in master mode? */
    if (I2C_PDD_GetTransmitMode(I2C1_BASE_PTR) == I2C_PDD_TX_DIRECTION) { /* Is device in Tx mode? */
        ...
        if ((DeviceDataPrv->SerFlag & ADDR_COMPLETE) != 0x00U) { /* If 10-bit addr has been completed */
          if (DeviceDataPrv->OutLenM != 0x00U) { /* Is any char. for transmitting? */
            ...
            if (DeviceDataPrv->InpLenM != 0x00U) { /* Is any char. for reception? */
              ...
              I2C_Display_OnMasterBlockSent(DeviceDataPrv->UserData); /* Invoke OnMasterBlockSent event */
            }
          }
        } else {
      ...
      *(DeviceDataPrv->InpPtrM)++ = I2C_PDD_ReadDataReg(I2C1_BASE_PTR); /* Receive character */
      ...
      if (DeviceDataPrv->InpLenM == 0x00U) { /* Is any char. for reception? */
        I2C_Display_OnMasterBlockReceived(DeviceDataPrv->UserData); /* Invoke OnMasterBlockReceived event */
      }
    }
  } else {
  ...
  if (ErrorMask != 0x00U) {            /* Is any error mask set? */
    ...
    I2C_Display_OnError(DeviceDataPrv->UserData); /* If yes then invoke user event */
  }
}
...
LDD_TDeviceData* I2C_Display_Init(LDD_TUserData *UserDataPtr)
{
  ...
  I2C_Display_TDeviceData *DeviceDataPrv;
  ...
  DeviceDataPrv->SlaveAddr = 0xE8U;    /* Set variable for slave address */
  DeviceDataPrv->SendStop = LDD_I2C_SEND_STOP; /* Set variable for sending stop condition (for master mode) */
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_I2C_Display_ID,DeviceDataPrv);
...
void I2C_Display_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  PE_LDD_UnregisterDeviceStructure(PE_LDD_COMPONENT_I2C_Display_ID);
...
{
  I2C_Display_TDeviceData *DeviceDataPrv = (I2C_Display_TDeviceData *)DeviceDataPtr;
