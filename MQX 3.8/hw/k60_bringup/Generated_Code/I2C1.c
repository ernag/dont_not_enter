...
#define AVAILABLE_EVENTS_MASK (LDD_I2C_ON_MASTER_BLOCK_SENT | LDD_I2C_ON_MASTER_BLOCK_RECEIVED | LDD_I2C_ON_ERROR | LDD_I2C_ON_MASTER_BYTE_RECEIVED)
...
PE_ISR(I2C1_Interrupt)
{
  ...
  if (ErrorMask != 0x00U) {            /* Is any error mask set? */
    ...
    I2C1_OnError(DeviceDataPrv->UserData); /* If yes then invoke user event */
  }
}
...
LDD_TDeviceData* I2C1_Init(LDD_TUserData *UserDataPtr)
{
  ...
  DeviceDataPrv->SlaveAddr = 0x94U;    /* Set variable for slave address */
  DeviceDataPrv->SendStop = LDD_I2C_SEND_STOP; /* Set variable for sending stop condition (for master mode) */
  ...
  /* I2C1_F: MULT=1,ICR=0x1A */
  I2C1_F = (I2C_F_MULT(0x01) | I2C_F_ICR(0x1A)); /* Set prescaler bits */
...
bool I2C1_MasterGetBlockSentStatus(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  /* {Default RTOS Adapter} Critical section begin, general PE function is used */
  EnterCritical();
  Status = ((I2C1_TDeviceDataPtr)DeviceDataPtr)->StatusFlag; /* Save flag for return */
  ((I2C1_TDeviceDataPtr)DeviceDataPtr)->StatusFlag &= (uint8_t)(~(uint8_t)MASTER_BLOCK_SENT); /* Clear data block sent flag */
  /* {Default RTOS Adapter} Critical section end, general PE function is used */
  ExitCritical();
...
bool I2C1_MasterGetBlockReceivedStatus(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  EnterCritical();
  Status = ((I2C1_TDeviceDataPtr)DeviceDataPtr)->StatusFlag; /* Save flag for return */
  ((I2C1_TDeviceDataPtr)DeviceDataPtr)->StatusFlag &= (uint8_t)(~(uint8_t)MASTER_BLOCK_RECEIVED); /* Clear data block received flag */
  /* {Default RTOS Adapter} Critical section end, general PE function is used */
  ExitCritical();
