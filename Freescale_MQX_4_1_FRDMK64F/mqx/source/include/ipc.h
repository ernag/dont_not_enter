...
typedef _mqx_uint (_CODE_PTR_ IPC_INIT_FPTR)( 
        const struct ipc_protocol_init_struct      *ipc_init_ptr, void   *ipc_info_ptr);
typedef _mqx_uint (_CODE_PTR_ IPC_HANDLER_FPTR)( void *);
...
extern _mqx_uint _ipc_msg_route_add(_processor_number, _processor_number,
   _queue_number);
extern void     *_ipc_msg_processor_route_exists(_processor_number);
extern _mqx_uint _ipc_msg_route_remove(_processor_number, _processor_number,
   _queue_number);
extern void      _ipc_task(uint32_t);
#endif
