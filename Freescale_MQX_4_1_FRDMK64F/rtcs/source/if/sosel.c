...
{ /* Body */
   ...
   for (parms = socket_cfg_ptr->SELECT_HEAD; parms; parms = nextparms)
   {
      ...
      #endif
      {
        ...
        /* flag is expected to be 1 only by SOCK_STREAM_send */
        /* where it is waiting for ACK on one socket.   */
        /* in this case unblock socket layer immediately */
         if((!flag)&&(0xFFFFFFFF!=parms->sock_count))
         {
